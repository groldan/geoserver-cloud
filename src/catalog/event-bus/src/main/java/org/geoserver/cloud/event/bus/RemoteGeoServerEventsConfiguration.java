...
public class RemoteGeoServerEventsConfiguration {
    ...
    /**
     * Add a {@link GeoToolsFilterModule} to the default jackson spring codecs if
     * not already present, so {@link Expression} and {@link Filter} objects can be
     * used as {@link RemoteGeoServerEvent} payload, especially {@link Literal}
     * expressions.
     */
    @Bean
    ...
    /**
     * Add a {@link GeoServerCatalogModule} to the default jackson spring codecs if
     * not already present, so {@link CatalogInfo} objects can be used as
     * {@link RemoteGeoServerEvent} payload
     */
    @Bean
    ...
    /**
     * Add a {@link GeoServerConfigModule} to the default jackson spring codecs if
     * not already present, so configuration {@link Info} objects can be used as
     * {@link RemoteGeoServerEvent} payload
     */
    @Bean
    ...
    /**
     * Add a {@link GeoServerCloudCatalogModule} to the default jackson spring
     * codecs if not already present, so configuration {@link Patch} and
     * {@link Query} objects can be used in {@link RemoteGeoServerEvent} payload
     */
    @Bean
     ...
     *
     * <p>
     * This listener ensures the payload object properties are resolved before being
     * catch up by other listeners.
     */
    @Bean
