...
public class EventualConsistencyEnforcer implements InitializingBean, DisposableBean {
    ...
    int monitorExecutionPeriodSeconds = 5;

    private EventualConsistencyHashConvergence hasher = new EventualConsistencyHashConvergence();

    private boolean notifyConvergence;
    private boolean hashConverged = false;
    private Instant convergedAt = Instant.now();
    private Instant lastUpdateAt = Instant.now();
    private long updatesSinceLastConvergence = 0L;
    ...
    private void initMonitor() {
        var tf = new CustomizableThreadFactory();
        tf.setDaemon(true);
        tf.setThreadNamePrefix("datadir-eventual-consistency-monitor-");
        int initialDelay = 10;
        int period = monitorExecutionPeriodSeconds;
        monitorExecutor = Executors.newScheduledThreadPool(1, tf);
        monitorExecutor.scheduleWithFixedDelay(this::checkPending, initialDelay, period, SECONDS);
    }
    ...
    private void converge(CatalogInfo info) {
        ...
        hashConverged = false;
        ++updatesSinceLastConvergence;
    }

    /**
     * Called periodically from the {@code datadir-eventual-consistency-monitor} thread to check if
     * there are still pending inserts waiting for dangling references
     */
    ...
    private void resolvePending() {
        if (pendingOperations.size() > 0) {
            notifyConvergence = true;
            for (String missingRef : Set.copyOf(pendingOperations.keySet())) {
                tryResolvePending(missingRef);
            }
        } else if (notifyConvergence) {
            log.info("data-directory catalog converged, there are no pending operations.");
            notifyConvergence = false;
        }
    }

    private void converge() {
        if (hashConverged) return;
        assert updatesSinceLastConvergence > 0;
        Duration sinceLastUpdate = Duration.between(lastUpdateAt, Instant.now());
        Duration sinceConverged = Duration.between(convergedAt, Instant.now());
        if (sinceLastUpdate.toMillis() < 1000 && updatesSinceLastConvergence >= 10) {
            double rate =
                    ((double) updatesSinceLastConvergence / sinceConverged.toMillis()) * 1000d;
            log.info(
                    "delaying hash convergence, catalog seems to be under stress: %,2d updates in the last %1f seconds (%2f/s)"
                            .formatted(
                                    updatesSinceLastConvergence,
                                    sinceConverged.toMillis() / 1000d,
                                    rate));
            return;
        }
        log.info(
                "converging, changes: %,2d. Last convergence: %,2d seconds ago."
                        .formatted(updatesSinceLastConvergence, sinceConverged.toSeconds()));
        try {
            hasher.converge(rawFacade);
        } finally {
            hashConverged = true;
            updatesSinceLastConvergence = 0;
            convergedAt = Instant.now();
            lastUpdateAt = convergedAt;
        }
    }
    ...
    private void tryResolvePending(CatalogInfo resolved) {
        ...
        for (var op : pending) {
            log.trace("converging operation for resolved {}: {}", resolved.getId(), op);
            execute(op);
        }
    }
    ...
    private <T> T execute(ConsistencyOp<T> op) {
        ...
        try {
            T ret = op.call();
            if (!op.completedSuccessfully() && log.isInfoEnabled()) {
    ...
    private <T extends CatalogInfo> Set<String> findMissingRefs(T info) {
    ...
    private abstract class ConsistencyOp<T> implements Callable<T> {
        ...
        public final T call() {
            ...
            log.debug("deferring {}, missing {}", this, unresolved);
        ...
        private void setPending(String missingRef, ConsistencyOp<?> deferredOp) {
            log.trace("missing ref {}, deferring execution of {}", missingRef, deferredOp);
        ...
        private void unsetPending(String resolvedRef, ConsistencyOp<?> completedOp) {
            log.trace("missing ref {} resolved, completed {}", resolvedRef, completedOp);
        ...
        protected void discard(ConsistencyOp<?> op) {
            for (String ref : Set.copyOf(pendingOperations.keySet())) {
                ...
                if (removed) {
                    log.trace("discarded op waiting for ref {}: {}", ref, op);
                }
                if (pending.isEmpty()) {
    ...
    private class UpdateOp<T extends CatalogInfo> extends ConsistencyOp<T> {
        ...
        protected T resolve() {
            ...
            if (found.isPresent()) {
                ...
                if (missing.isEmpty()) {
                    success = true;
                    T updated = rawFacade.update(toUpdate, patch);
                    converge(updated);
                    return updated;
                }
            }
