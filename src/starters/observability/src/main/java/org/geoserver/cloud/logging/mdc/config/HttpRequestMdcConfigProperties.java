...
public class HttpRequestMdcConfigProperties {
    ...
    public HttpRequestMdcConfigProperties cookies(
            Supplier<MultiValueMap<String, HttpCookie>> cookies) {
        if (isCookies()) {
    ...
    private void putHeader(String name, List<String> values) {
        if (includeHeader(name)) {
            put(
                    "http.request.header.%s".formatted(name),
    ...
    public HttpRequestMdcConfigProperties parameters(
            Supplier<MultiValueMap<String, String>> parameters) {
        if (isParameters()) {
